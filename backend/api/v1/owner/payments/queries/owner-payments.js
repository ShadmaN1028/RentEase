...
const queries = {
  ...
  (f.rent - IFNULL(SUM(p.amount), 0)) AS amount_left,
  CASE 
    WHEN (f.rent - IFNULL(SUM(p.amount), 0)) = 0 THEN 'Paid'
    WHEN IFNULL(SUM(p.amount), 0) = 0 THEN 'Due'
    ELSE 'Partially Paid'
  END AS payment_status
FROM tenancies t
JOIN users u ON t.user_id = u.user_id
JOIN flats f ON t.flats_id = f.flats_id
LEFT JOIN payments p ON t.tenancy_id = p.tenancy_id
WHERE t.owner_id = ? AND t.status = 1
GROUP BY t.tenancy_id, u.first_name, u.last_name, f.flat_number, f.rent;
`,
  getPaymentDetails: `
