...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      const tenantDetails = await applications.getTenantDetails(tenant_id); // Or use ownerTenants if in a different file
      if (!tenantDetails) {
        return res
          .status(404)
          .json({ success: false, message: "Tenant not found" });
      }
      return res.status(200).json({ success: true, data: tenantDetails });
    } catch (error) {
